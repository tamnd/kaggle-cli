...
describes what users get. Pick a format with `-o`, or let kaggle choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kaggle <command> -o table   # aligned columns for reading
kaggle <command> -o jsonl   # one JSON object per line, for piping
kaggle <command> -o json    # a single JSON array
kaggle <command> -o csv     # spreadsheet friendly
kaggle <command> -o tsv     # tab-separated
kaggle <command> -o url     # just the URL column
kaggle <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kaggle <command> --fields id,title,url
```
...
```bash
kaggle <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kaggle <command>            # a table, because this is a terminal
kaggle <command> | wc -l    # JSONL, because this is a pipe
```
