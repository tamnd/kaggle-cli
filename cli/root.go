...
func Root() *cobra.Command {
	app := &App{cfg: kaggle.DefaultConfig()}

	root := &cobra.Command{
		Use:   "kaggle",
		Short: "Browse Kaggle datasets and competitions",
		Long: `kaggle reads public Kaggle data through the official API.

Dataset search is available without authentication. Competition data
requires a Kaggle account (set KAGGLE_USERNAME and KAGGLE_KEY).

kaggle is an independent tool and is not affiliated with Kaggle Inc.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = kaggle.NewClient(a.cfg)
	return nil
}
